...
public class TrackingAPI {
	...
	CheckInOperationService cehckInOperationService;
		...
		try{
			if(null != checkInrecord){
				if(null!= checkInrecord.getLat()|| null!= checkInrecord.getLong_() || null!= checkInrecord.getModifiedDateTime() || null != checkInrecord.getUserDeatil()){
					...
					cehckInOperationService = new CheckInOperationServiceImpl();
					Tracking trackRecord = cehckInOperationService.checkInLocation(checkInrecord);
	...
	public Response lastCheckInForUser(@PathParam("userID") Integer userID ) {
		try{
			if(null != userID){
					...
					cehckInOperationService = new CheckInOperationServiceImpl();
					Tracking lastCheckedIn = cehckInOperationService.getUsersLastLocation(userID);
